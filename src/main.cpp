...
#define FULL_UPDATE_FILE_PATH   "/sd/firmware.bin"
#define POST_APPLICATION_ADDR 0x08015000
//Pin order: MOSI, MISO, SCK, CS
SDBlockDevice sd(PA_7, PA_6,
                 PA_5, PA_4);
...
int main()
{
    sd.init();
    fs.mount(&sd);
    FILE *file = fopen("/sd/firmware.bin", "rb");
    if (file != NULL) {
